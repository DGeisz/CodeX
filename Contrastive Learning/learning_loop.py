...
def aug_learning_loop_single_probe(model, dataset, probe_data, output_dir, delete_existing_dir=False):
    checkpoint_path = f"./checkpoint_{output_dir}/"
    checkpoint_dir = Path(checkpoint_path)
    ...
    for epoch in range(start_epoch, EPOCHS):
        for step, ((y1, y2),  _) in enumerate(loader, start=epoch * len(loader)):
            ...
            if step % PRINT_FREQ == 0:
                ...
                eigen = dict(eigenvalues=model.cov_eig(p1, p2))
                end = time.time()
                print('Ending Eigen Comp', end - start, "sec")
...
def aug_learning_loop(model, dataset, output_dir, delete_existing_dir=False):
    checkpoint_path = f"./checkpoint_{output_dir}/"
    checkpoint_dir = Path(checkpoint_path)
